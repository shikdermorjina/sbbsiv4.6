/*
# Edit Invoice Function (Full Edit with Reversal)

1. New Function
- `edit_invoice(p_invoice_id uuid, p_new_data json, p_reason text, p_edited_by text)`
  - Performs a full edit of any non-draft, non-cancelled invoice by:
    a. Reversing ALL effects of the old invoice (stock, AR journal, COGS journal, payments)
    b. Updating the invoice header with new values (customer, dates, notes, totals)
    c. Deleting old invoice items and inserting new ones
    d. Triggers automatically fire for new items (stock deduction + COGS)
    e. Re-posting the AR journal entry for the new total
    f. Recording the edit in invoice_edit_history with before/after snapshots
    g. Updating customer outstanding balance
  - Returns JSON with success status

2. Parameters
- p_invoice_id: the invoice to edit
- p_new_data: JSON with keys: customer_id, invoice_date, due_date, notes, items[]
  - items[]: array of {product_id, quantity, unit_price, cost_price, discount_percent, unit_name, unit_conversion_factor, base_quantity}
- p_reason: mandatory reason for the edit
- p_edited_by: name of the user making the edit

3. Security
- SECURITY DEFINER — can modify journal_entries, stock_movements, payments, inventory_items
- Callable by anon and authenticated

4. Important Notes
- Cannot edit invoices with status 'cancelled'
- Cannot edit invoices that have been delivered (status = 'delivered' in deliveries)
- Cannot edit invoices with linked sales returns
- The old invoice's payments are reversed and new AR is posted for the new total
- Stock is restored for old items and re-deducted for new items (via triggers)
- This is a "reverse and recreate" strategy — the invoice keeps its ID and number
*/
...
BEGIN
  ...
  -- Temporarily set status to draft so item triggers don't double-post COGS
  -- We'll set the real status after items are inserted
  UPDATE invoices
  ...
  -- Insert new items (triggers will fire for stock deduction, but COGS is skipped for draft)
  FOR v_i IN SELECT generate_series(0, json_array_length(v_new_items) - 1) LOOP
    v_new_item := v_new_items->v_i;
  ...
  -- Set status back (this fires the invoice_accounting_trigger and invoice_status_cogs_trigger)
  -- which will post AR and COGS for the new items
  UPDATE invoices
  SET status = CASE
  ...
  IF v_invoice.status = 'paid' THEN
    ...
    -- Post a payment for the full amount
    INSERT INTO payments (
      payment_number, payment_type, payment_method, amount, payment_date,
      reference_type, reference_id, reference_number, notes, status
    ) VALUES (
