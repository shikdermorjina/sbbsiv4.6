...
-- ============================================================
-- FIX 1: cancel_invoice — allow cancellation even when sales
--         returns exist. The return JEs are self-contained.
--         We reverse all sales-return JEs and void the returns,
--         then proceed with normal invoice cancellation.
-- ============================================================
...
BEGIN
  ...
  FOR v_sr IN SELECT * FROM sales_returns WHERE invoice_id = p_invoice_id LOOP
    -- Reverse JEs tied to this sales return
    FOR v_je_id IN
    ...
    -- Reverse payment JEs for the return's refund payment
  ...
  FOR v_item IN SELECT * FROM invoice_items WHERE invoice_id = p_invoice_id LOOP
    ...
    -- Subtract already-returned quantity for this line item
...
-- ============================================================
-- FIX 2: edit_invoice — fix partially_paid status restoration.
-- When a partially_paid invoice is edited, after reversing the
-- old payments we re-create the payment at the existing
-- amount_paid so the invoice stays partially_paid correctly.
-- Also: keep the sales-returns block on edit (editing items
-- when returns exist would create item count mismatches).
-- ============================================================
...
BEGIN
  ...
  -- Keep block on edit when returns exist — item-level integrity
  ...
  -- Capture partial payment info before reversal
  ...
  IF v_invoice.status = 'paid' THEN
    -- Fully paid: re-create payment for new total
  ...
  ELSIF v_invoice.status = 'partially_paid' AND v_prev_amount_paid > 0 THEN
    -- Partially paid: re-create the partial payment record so the invoice
    -- remains partially_paid rather than falling back to 'sent'
  ...
  ELSE
    -- credit/unpaid/sent: keep original status
    UPDATE invoices
...
-- ============================================================
-- FIX 3: Check that sales_return_items has base_quantity_returned
--         column (used in stock deduction above). Add if missing.
-- ============================================================
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'sales_return_items' AND column_name = 'base_quantity_returned'
    AND table_schema = 'public'
  ) THEN
    ALTER TABLE sales_return_items ADD COLUMN base_quantity_returned numeric;
  END IF;
END $$;
